...
--
Building Restful API Microservices with python-flask, nameko, docker, rebbitMQ.

Deign:
	An In-Memory storage - dictionary.txt.gz

	UI (ruby file) --- Controller ----- add_service
			                |
			                --- delete_service
					|
					--- fetch_service
...
1. Download anagrams_project.tar file.
2. Uncompress it.
3. Go to anagrams_project directory.
4. Run it. (e.g: prompt> "docker-compose up". for detach, ""docker-compose up -d"). Make sure that docker-compose.yml exists.
5. prompt> "docker ps". Check if all services are up. Public port is 3000.
6. Test all curl operations or run "ruby anagram_test.rb"
7. To shut it down, prompt> "docker-compose down". Or Control-C


# Optional parts.
1. Endpoint that returns a count of words in the corpus and min/max/median/average word length
prompt> curl -i http://localhost:3000/statics

2. Endpoint that identifies words with the most anagrams
prompt> curl -i http://localhost:3000/anagrams/most

3. Endpoint that takes a set of words and returns whether or not they are all anagrams of each other
prompt> curl -i -X POST -d '{ "words": ["cat", "tac", "act"] }' http://localhost:3000/anagrams

4. Endpoint to return all anagram groups of size >= *x*
prompt> curl -i http://localhost:3000/anagrams/word.json?size=8

5. Endpoint to delete a word *and all of its anagrams*
prompt> curl -i -X DELETE http://localhost:3000/anagrams/read.json
